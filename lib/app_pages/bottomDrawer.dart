...
import 'db_model/table_timer.dart';
import 'db_model/db_helper.dart';
...
class BottomDrawerWidgetState extends State<BottomDrawerWidget> {
  ...
  List<AppModel> get passApps {
    return _timedApps;
  }

  Duration duration = const Duration(hours: 0, minutes: 0);

  Future<List<limitTimer>> fetchAppLimits() async {
    var dbHelper = db_helper();
    Future<List<limitTimer>> rawFetchedApps = dbHelper.getLimit();
    List<limitTimer> fetchedApps = await rawFetchedApps;
    for (var i = 0; i < fetchedApps.length; i++) {
      print(fetchedApps[i].appName);
      print(fetchedApps[i].limitTime);
    }
    return fetchedApps;
  }
                        ...
                        onPressed: () => Utils.showSheet(
                          ...
                          onClicked: () {
                            setState(() {
                              ...
                              var dbhelper = db_helper();
                              if (_finalApps[i].isSet) {
                                dbhelper.updateLimit(limitTimer(
                                    _finalApps[i].title,
                                    _finalApps[i].appLimit));
                              } else {
                                dbhelper.insertLimit(
                                  limitTimer(_finalApps[i].title,
                                      _finalApps[i].appLimit),
                                );
              ...
              itemBuilder: (BuildContext context, int i) => Column(
                children: [
                  Container(
                    ...
                    child: ListTile(
                      leading: Image.memory(_noUsedApps[i].icon),
                      title: Text(_noUsedApps[i].title),
                      subtitle: const Text("No longer used"),
                      trailing: IconButton(
                        icon: Container(
